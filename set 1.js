...
function savings(grossPay, taxRate, expenses) {
 ...
 //Calculation for Money Remaining After Taxes and Expenses
    let salaryAfterDeduction = grossPay * (1-taxRate)

//Round down of salary after tax 
    salaryAfterDeduction = (Math.floor(salaryAfterDeduction));

//Final Calculation of Salary After Expenses 
    let savings = salaryAfterDeduction - expenses
...
function materialWaste(totalMaterial, materialUnits, numJobs, jobConsumption) {
    ...
    // Calculate How Much Material Consumed Per Job 
        let totalConsumed = numJobs * jobConsumption

    //Calculate Remainder of Material Available 
        let Remainder = totalMaterial - totalConsumed

    //Return Remaining Material (Do not add a space between the number and the unit.)
        return Remainder + materialUnits;
}
...
function interest(principal, rate, periods) {
    ...
    // Calculate income earned from investment 
    let gainedFromInvestment=principal * (rate * periods);

    let finalValueOfInvestment = principal + gainedFromInvestment
